...
function FreePost(props) {
    ...
    useEffect(
        () => {
            axios({
                url: '/oauth2/redirect/fboard/list/',
                method: 'GET'
            }).then((res) => {
                callback(res.data);
                console.log(res);
                    ...
                    filterArray.push(
                        <div>
                            ...
                            <tr key={val.id} className="content">
                                ...
                                {/* 글에 해당하는 글 주소 집어넣기 */}
                                <td className="title" onClick={() => { history.push("/freepostdetail/" + `${val.id}`) }}>{val.title}<strong></strong></td>
    ...
    */
    function page() {
        ...
        for (let i = 1; i <= ((testStr.length) + 9) / 10; i++) {// 한 페이지 별 글 세개씩 -> 데이터 개수+2 / 3
            pageNum.push(<button className="page" onClick={() => { history.push("/freepost/" + i) }} >{i}</button>);
    ...
    function prev() {
        ...
        let prevPage = id - 1;
        if (id > 1) {
            prevNum.push(<button className="prev" onClick={() => { history.push("/freepost/" + prevPage) }} >이전</button>)
        }
        return prevNum;
    }
    ...
    function next() {
        ...
        if (id < maxPage) {
            nextNum.push(<button className="next" onClick={() => { history.push("/freepost/" + nextPage) }} >다음</button>)
        }
        ...
        <div className="post">
            <div className="post-box">
                <div className="top">
                    ...
                    <div className="head">
                        <div className="title">
                            ...
                            <span className='second'>자유게시판</span>
                        </div>
                ...
                <div className="bottom">
                    ...
                    <Link to="/writefreepost"><button className="writeBtn">글쓰기</button></Link>
